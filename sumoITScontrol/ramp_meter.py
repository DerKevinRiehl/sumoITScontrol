...
class RampMeter:
    def __init__(self, tl_id, queue_sensors, mainline_sensors, smoothening_factor=0.1, saturation_flow_veh_per_sec=0.5):
        self.tl_id = tl_id
        ...
        for sensor in self.mainline_sensors:
            if self.mainline_sensor_types[sensor]=="inductionloop":
                if state_func=="getLastIntervalOccupancy":
                    measurement_data.append(traci.inductionloop.getLastIntervalOccupancy(sensor))
            else:
                if state_func=="getLastIntervalOccupancy":
                    measurement_data.append(traci.lanearea.getLastIntervalOccupancy(sensor))
        ...
        for sensor in self.queue_sensors:
            if self.queue_sensor_types[sensor]=="inductionloop":
                if state_func=="getLastIntervalMaxJamLengthInMeters":
                    measurement_data.append(traci.inductionloop.getLastIntervalMaxJamLengthInMeters(sensor))
                if state_func=="getLastIntervalVehicleNumber":
                    measurement_data.append(traci.inductionloop.getLastIntervalVehicleNumber(sensor))
            else:
                if state_func=="getLastIntervalMaxJamLengthInMeters":
                    measurement_data.append(traci.lanearea.getLastIntervalMaxJamLengthInMeters(sensor))
                if state_func=="getLastIntervalVehicleNumber":
                    measurement_data.append(traci.lanearea.getLastIntervalVehicleNumber(sensor))
        return measurement_data
    
    def _update_ramp_signal_control_logic_alinea(self, metering_rate: float, cycle_duration: int):
        """
    ...
    def get_smoothened_flow(self):
        ...
        self.flow_smoothed = (self.smoothening_factor * flow + (1 - self.smoothening_factor) * self.flow_smoothed)
