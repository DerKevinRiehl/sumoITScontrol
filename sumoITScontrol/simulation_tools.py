...
class SimulationTools:
    ...
    def determine_hidden_vehicles(traci):
        if SimulationTools.hidden_vehicle_update_time==-1 or SimulationTools.hidden_vehicle_update_time!=traci.simulation.getCurrentTime():
            SimulationTools.hidden_vehicle_update_time = traci.simulation.getCurrentTime()
            ...
            hidden_vehicles_routes = [traci.vehicle.getRoute(v_id) for v_id in SimulationTools.hidden_vehicles]
            hidden_vehicles_routes_index = [traci.vehicle.getRouteIndex(v_id) for v_id in SimulationTools.hidden_vehicles]
