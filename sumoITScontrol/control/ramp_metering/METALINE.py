# sumoITScontrol/control/ramp_metering/METALINE.py
...
class METALINE:
    def __init__(self, params, coordination_group, target_occupancies, K_P, K_I=None):
        ...
        # Measurement data
        self.measurement_data = {rm_id: {"occupancy": [], "queue_length_m": [], "metering_rate": []} 
    ...
    def execute_control(self, current_time):
        ...
        if self.counter == self.params["measurement_period"]:
            ...
            for i, ramp_meter in enumerate(self.group.ramp_meters):
                occupancies[i] = np.nanmean(ramp_meter._get_mainline_state("getLastIntervalOccupancy"))
                queues[i] = np.nanmean(ramp_meter._get_queue_state("getLastIntervalMaxJamLengthInMeters"))
            # METALINE control law (vectorized)
            rates = self.prev_rates + self.K_P @ (self.target_occupancies - occupancies) \
                    + self.K_I @ (occupancies - self.prev_occupancies)
            # Saturate to min/max
            rates = np.clip(rates, self.params.get("min_rate", 5), self.params.get("max_rate", 100))            
            # Apply metering
            for i, ramp_meter in enumerate(self.group.ramp_meters):
                ...
                self.measurement_data[rm_id]["occupancy"].append([current_time, occupancies[i]])
                self.measurement_data[rm_id]["queue_length_m"].append([current_time, queues[i]])
                self.measurement_data[rm_id]["metering_rate"].append([current_time, rates[i]])
