...
class MaxPressure_Fix:
    def __init__(self, params, intersection):
        ...
        self.measurement_data["schedule"] = []  # green durations (seconds) for each phase in order
    ...
    def _compute_schedule_from_pressures(self):
        ...
        self.measurement_data["pressures"] = [
            sum(window) / len(window) for window in self.measurement_data["pressures_hist"]
        ]
        self.measurement_data["pressures_hist"] = []
        ...
        total_pressure = sum(self.measurement_data["pressures"]) if self.measurement_data["pressures"] else 0
        greens = []
        if total_pressure <= 0:
            # distribute equally
        ...
        if total_alloc > effective_green and effective_green > 0:
            ...
            greens = [max(self.params["G_T_MIN"], min(equal, self.params["G_T_MAX"])) for _ in greens]
    ...
    def execute_control(self, current_time):
        ...
        if self.measurement_data["counter"] == self.params["measurement_period"]:
            ...
            self.measurement_data["fsm_timer"] += 1
            self.measurement_data["pressures_hist"].append(self.intersection.get_queue_lengths_num_vehicles())
            # when starting a new cycle (schedule index 0 and not currently in green), compute schedule
            if self.measurement_data["schedule_index"] == 0 and self.measurement_data["current_fsm_state"] in ("idle", "transition"):
            ...
            if self.measurement_data["current_fsm_state"] == "green":
                ...
                green_duration = schedule[idx] if idx < len(schedule) else self.params["G_T_MIN"]
                ...
                if current_gt >= green_duration:
                    ...
                    self.measurement_data["next_signal_phase"] = int(self.measurement_data["current_signal_phase"] + 1)
                    self.measurement_data["current_signal_phase"] = self.measurement_data["current_signal_phase"] + 1
                    self.measurement_data["fsm_timer"] = 0
            ...
            if self.measurement_data["current_fsm_state"] == "transition":
                current_t = self.measurement_data["fsm_timer"] * (1)  # timer counted in measurement ticks; treat as seconds
                if current_t >= self.params["T_L"]:
                    # advance to next schedule index
                    self.measurement_data["schedule_index"] += 1
                    if self.measurement_data["schedule_index"] >= len(self.intersection.phases):
