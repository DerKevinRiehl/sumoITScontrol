...
plt.step(np.asarray(controller.measurement_data["metering_rate"])[:,0],
         np.asarray(controller.measurement_data["metering_rate"])[:,1],)

plt.subplot(1,3,2)
plt.step(np.asarray(controller.measurement_data["metering_occupancy"])[:,0],
         np.asarray(controller.measurement_data["metering_occupancy"])[:,1],)
plt.xlabel("Simulation Time [s]")
plt.ylabel("Mainline State (Occupancy [%])")

plt.subplot(1,3,3)
plt.step(np.asarray(controller.measurement_data["queue_length_m"])[:,0],
